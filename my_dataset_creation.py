...
def generate_density_map(image_size, box_sizes): #box_sizes=[[sx, ex, sy, ey], ...]
    ...
    for i in range(0, len(box_sizes)):
        ...
        print("bounding box shape: ", bounding_box + density_map[box_sizes[i][2]:box_sizes[i][3], box_sizes[i][0]:box_sizes[i][1]])'''
...
for img_path in tqdm(img_paths):
    ...
    gt_path = img_path.replace("images", "jsons").replace(".jpg", ".json")
    ...
    with h5py.File(img_path.replace('.jpg', 'dot.h5').replace('images', 'mydataset'), 'w') as hf:
